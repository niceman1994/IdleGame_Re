...
public class ItemDrop : MonoBehaviour
{
    ...
    public void CreateAllItem(Transform parent, List<GameObject> itemList)
    {
    ...
    public void SpawnItem(Vector3 spawnPosition)
    {
        ...
        if (currentItemPercent <= 100 && currentItemPercent > 90)
    ...
    private void MakeItem(Vector3 pos, GameObject item)
    {
        item.SetActive(true);
        item.transform.position = new Vector3(pos.x + 1.4f, pos.y - 1.25f, pos.z);
    }

    private void MakeItem(float _x, float _y, float _z, GameObject item)
    {
        item.SetActive(true);
